...
// 작성하기 버튼 눌렀을때 :)
  ...
  return (
    <FormWrap>
      ...
      <label>만화제목 <input type='text' ref={title_ref} defaultValue='' placeholder="추억 속 만화 제목을 적어주세요"/></label>
      <label>방영연도 <input type='number' ref={onair_year_ref} defaultValue='2000'/></label>
      <label>만화이미지 <input type='file' ref={thumbnail_ref} defaultValue=''/></label>
      <label>만화 OST <input type='url' ref={ost_url_ref} defaultValue='' placeholder="추억의 주제가 링크를 넣어주시겠어요? (필수 x)"/></label>
      ...
      <label>만화소개</label> <textarea ref={content_ref} defaultValue='' placeholder="당신의 추억 속 이 만화는 어떤 만화였나요? "/>
